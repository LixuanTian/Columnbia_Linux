...
QUERY::QUERY(CString QueryFile) {
  ...
  if ((fp = fopen(QueryFile.c_str(), "r")) == NULL)
  ...
  while (IsCommentOrBlankLine(TextLine)) {
    ...
    if (feof(fp))
      OUTPUT_ERROR("no query expression!");
  }
  ...
  if (*p != LEFT_BRACKET)
    OUTPUT_ERROR("Expression must start with '('. In batch mode, expression "
                 "must initiate with 'Query:'   ");
  ...
  for (;;) {
    ...
    if (i + 1 >= MAXLENGTH)
      OUTPUT_ERROR("Query Length Exceeds MAXLENGTH limit");
  }
  ...
  PTRACE("initial query tree: %s", QueryExpr->Dump().c_str());
...
EXPR *QUERY::ParseExpr(char *&ExprStr) {
  ...
  if (p = strstr(OneElement, KEYWORD_EQJOIN)) // EQJOIN
  {
    ...
    // if the eqjoin is on multiattribute, choose those attributes that have max
    // CuCard
    if (Size > 1) {
  ...
  if (p = strstr(OneElement, KEYWORD_FUNC_OP)) // FUNC_OP
  {
    ...
    // get RangeVar
    while (*p != RIGHT_BRACKET && *p != COMMA && *p != BLANKSPACE &&
           *p != TABSPACE)
  ...
  if (p = strstr(OneElement, KEYWORD_ORDER_BY)) // ORDER_BY
  {
    ...
    if (*p == ')') {
      for (int i = 0; i < KeysSet->GetSize(); i++)
        sort_prop->KeyOrder.Add(ascending);
    } else {
      char *str;
      ...
      while (*p != '>') {
        ...
        if (*p == '>') {
          *p = 0; // get a str
          str = SkipSpace(str);
          sort_prop->KeyOrder.Add(
              atoKeyOrder(str)); // add the order to KeyOrder
          break;
        }
        if (*p == ',') {
          *p = 0; // get a str
...
void QUERY::GetKey(char *&p, KEYS_SET &Keys) {
  ...
  while (*p != DOT && *p != RIGHT_BRACKET && *p != COMMA)
    Col += *p++;
  ...
  p++; // skip the '.'
  while (*p != RIGHT_BRACKET && *p != COMMA && *p != '>' && *p != BLANKSPACE &&
         *p != TABSPACE)
    Attr += *p++;
...
CString QUERY::Dump_IntOrders() { return (IntOrdersSet.Dump()); }
