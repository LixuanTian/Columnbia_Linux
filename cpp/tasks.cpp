...
bool PTASKS::empty() { return (first == NULL); } // TASK_LIST::empty
    ...
    Cases (3) and (4) remain.  More search.
            ...
            if (property is ANY)
                    Push O_EXPR on first logical expression
                            add a winner for this context to the circle, with
null plan (i.e., initialize the winner's circle for this property.) else Push
O_GROUP on this group with current context Push O_GROUP on this group, with new
context: ANY property and cost = current context cost - appropriate enforcer
cost, last task. This is valid since the result of the ANY search will only be
used with the enforcer.  If the cost is negative, then the enforcer cannot be
used, so the ANY winner should be null. Since this is somewhat complex and prone
to error, we will omit it for now and use the original context's cost else
(Group is optimized) if (property is ANY) assert (this is case 4) push O_INPUTS
on all physical mexprs else (property is not ANY) Push O_INPUTS on all physical
mexprs with current context, last one is last task If case (3) [i.e. appropriate
enforcer is not in group], Push APPLY_RULE on enforcer rule, not the last task
                            add a winner for this context to the circle, with
null plan. (i.e., initialize the winner's circle for this property.)
...
void O_GROUP::perform() {
  ...
  PHYS_PROP *LocalReqdProp =
      M_WINNER::mc[GrpID]->GetPhysProp(ContextID); // What prop is required of
  ...
  if (!Group->is_optimized()) {
    ...
    if (LocalReqdProp->GetOrder() == any) {
      PTRACE(
          "%s",
          "add winner with null plan, push O_EXPR on 1st logical expression");
  ...
  } else // Group is optimized
  {
    ...
    if (LocalReqdProp->GetOrder() == any) {
      ...
      for (M_EXPR *PhysMExpr = Group->GetFirstPhysMExpr(); PhysMExpr;
      ...
      while (--count >= 0) {
        ...
        if (GlobepsPruning) {
          ...
          PTasks.push(
              new O_INPUTS(PhysMExprs[count], ContextID, TaskNo, false));
      }
    } else // property is not ANY)
    {
      ...
      // If case (3) [i.e. appropriate enforcer is not in group], Push
      // APPLY_RULE on 	enforcer rule, not the last task
      if (!SCReturn) {
        PTRACE("%s", "Push APPLY_RULE on enforcer rule");
        if (LocalReqdProp->GetOrder() == sorted) {
          ...
          if (GlobepsPruning) {
            COST *eps_bound = new COST(*EpsBound);
            PTasks.push(
                new APPLY_RULE(Rule, FirstLogMExpr, false, ContextID, TaskNo));
          } else
            PTasks.push(new APPLY_RULE(Rule, FirstLogMExpr, false, ContextID,
                                       TaskNo, false));
        } else {
          assert(false);
        }
...
E_GROUP::E_GROUP(GRP_ID grpID, int ContextID, int parentTaskNo, bool last,
                 COST *bound)
    : TASK(ContextID, parentTaskNo), GrpID(grpID), Last(last), EpsBound(bound) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_E_GROUP].New();
}; // E_GROUP::E_GROUP
...
void E_GROUP::perform() {
  ...
  if (Group->is_exploring())
    ...
    // this logical mexpr will be the last optimized one, mark it as the last
    // task for this group
    if (GlobepsPruning) {
      COST *eps_bound = new COST(*EpsBound);
      PTasks.push(
          new O_EXPR(LogMExpr, true, ContextID, TaskNo, true, eps_bound));
    } else
...
O_EXPR::O_EXPR(M_EXPR *mexpr, bool explore, int ContextID, int parent_task_no,
               bool last, COST *bound)
    : TASK(ContextID, parent_task_no), MExpr(mexpr), explore(explore),
      Last(last), EpsBound(bound) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_O_EXPR].New();
}; // O_EXPR::O_EXPR

// ##ModelId=3B0C085E0041
void O_EXPR::perform() {
  PTRACE2("O_EXPR performing, %s mexpr: %s ",
          explore ? "exploring" : "optimizing", MExpr->Dump().c_str());
#ifdef IRPROP
  int GrpNo = MExpr->GetGrpID();
  PTRACE2("ContextID: %d, %s", ContextID,
          (M_WINNER::mc[GrpNo]->GetPhysProp(ContextID))->Dump().c_str());
#else
  PTRACE2("Context ID: %d , %s", ContextID,
          CONT::vc[ContextID]->Dump().c_str());
#endif
  ...
  if (explore)
    assert(
        MExpr->GetOp()->is_logical()); // explore is only for logical expression
      ...
      if (original->GetInput(input_no)->GetArity()) {
        ...
        if (!Ssp->GetGroup(grp_no)->is_exploring()) {
          ...
          if (GlobepsPruning) {
            COST *eps_bound = new COST(*EpsBound);
            PTasks.push(
                new E_GROUP(grp_no, ContextID, TaskNo, false, eps_bound));
          } else
...
O_INPUTS::O_INPUTS(M_EXPR *MExpr, int ContextID, int ParentTaskNo, bool last,
                   COST *bound, int ContNo)
    : MExpr(MExpr), TASK(ContextID, ParentTaskNo), InputNo(-1), Last(last),
      PrevInputNo(-1), EpsBound(bound), ContNo(ContNo) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_O_INPUTS].New();
...
There are still three flags: Pruning (also called Group Pruning), CuCardPruning
and GlobepsPruning, with new meanings.  We plan to run benchmarks in four cases:

1. Starburst - generate all expressions [!Pruning && !CuCardPruning]
2. Group Pruning - aggressively check limits at all times [Pruning &&
!CuCardPruning] aggressively check means if ( CostSoFar >= upper bound of
context in G) then terminate.
3. Lower Bound Pruning - if there is no winner, then use IG's SLB in
InputCost[]. [CuCardPruning]. This case assumes that the Pruning flag is on,
i.e. the code forces Pruning to be true if CuCardPruning is true.  The SLB may
involve cucard, fetch, copy, etc in the lower bound.
4. Global Epsilon Pruning [GlobepsPruning].  If a plan costs <= GLOBAL_EPS, it
is a winner for G.

PSEUDOCODE

On the first (and no other) execution, the code must initialize some O_INPUTS
members. The idea here is to get a quick lower bound for the cost of the inputs.
...
else if (!CuCardPruning) //Group Pruning case (since Starburst not relevant
here) InputCost[IG] = 0
//remainder is Lower Bound Pruning case
      ...
      InputCost[IG] = max(cost of winner, IG's SLB) //This is a lower bound for
IG else error - previous cost failed because of property
...
If Starburst case and case (1)
      ...
      push this task
      push O_GROUP for IG, using current context's cost minus CostSoFar plus
InputCost[InputNo] terminate this task else // we just returned from O_GROUP on
IG Trace: This is an impossible plan terminate this task InputNo++; endFor
...
else If (winner is non-null and CostSoFar >= cost of this winner) || winner is
null ) Replace existing winner with current mexpression and its cost, don't
change done flag Update the upper bound of the current context
*/

// ##ModelId=3B0C085E0307
void O_INPUTS::perform() {
  PTRACE2("O_INPUT performing Input %d, expr: %s", InputNo,
          MExpr->Dump().c_str());
#ifdef IRPROP
  int GrpNo = MExpr->GetGrpID();
  PTRACE2("ContextID: %d, %s", ContextID,
...
#else
  PHYS_PROP *LocalReqdProp =
      CONT::vc[ContextID]->GetPhysProp(); // What prop is required
  ...
  if (GlobepsPruning && CONT::vc[ContextID]->is_done()) {
    ...
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_O_INPUTS].Delete();
    if (Last)
      // this's the last task for the group, so mark the group with completed
      // optimizing
  ...
  // On the first (and no other) execution, code must initialize some O_INPUTS
  // members. The only nontrivial member is InputCost.
  if (InputNo == -1) {
    // init inputLogProp
    for (input = 0; input < arity; input++) {
      GROUP *InputGroup =
          Ssp->GetGroup(MExpr->GetInput(input)); // Group of current input
    ...
    for (input = 0; input < arity; input++) {
      ...
      if (IG->GetFirstLogMExpr()->GetOp()->is_const()) {
        ...
        InputCost[input] =
            ((CONST_OP *)IG->GetFirstLogMExpr()->GetOp())->get_cost();
        continue;
      }
      ...
      if (Op->is_physical()) {
        ...
        ReqProp = ((PHYS_OP *)Op)
                      ->InputReqdProp(LocalReqdProp, InputLogProp[input], input,
                                      possible);

        if (!possible) // if not possible, means no such input prop can
                       // satisfied
        {
      ...
      // else if (!CuCardPruning) //Group Pruning case (since Starburst not
      // relevant here) InputCost[IG] = 0
    ...
    InputNo++; // Ensure that previous code will not be executed again; begin
               // with Input 0
  }

  // If Global Pruning and cost so far is greater than upper bound for this
  // context, then terminate
  ...
  for (input = InputNo; input < arity; input++) {
    ...
    if (IG->GetFirstLogMExpr()
            ->GetOp()
            ->is_const()) { // the cost of item group will not be refined,
                            // always equal to ConstGroupCost (0)
    ...
    if (Op->is_physical()) {
      ...
      if (Pruning)
        assert(possible); // should be possible since in the first pass, we
                          // checked it
      if (!possible) {
        delete ReqProp;
        goto TerminateThisTask;
    ...
    // group is not optimized
             ...
             PrevInputNo) // no winner, and we did not just return from O_GROUP
    {
      PTRACE("No Winner for Input : %d", input);

      // Adjust PrevInputNo and InputNo to track progress after returning from
      // pushes
      PrevInputNo = input;
      InputNo = input;
      ...
      COST *InputBd =
          new COST(*LocalUB); // Start with upper bound of G's context
      if (Pruning) {
      ...
      PTasks.push(new O_GROUP(
          IGNo, 0, TaskNo,
          true)); // pass context as "any", as the group is not at all optimized
  ...
  // All inputs have been been optimized, so compute cost of the expression
  // being optimized.
  ...
  // HaltWinSize winners is <= HaltImpr. This only works for EQJOIN
#ifndef IRPROP
  if (Halt) {
    if (LocalGroup->GetFirstLogMExpr()->GetOp()->GetName().Compare("EQJOIN") ==
        0) {
      ...
      // if count>halt_size, we can stop here, and set the current winner as
      // final winner
  ...
  if ((M_WINNER::mc[GrpNo]->GetBPlan(LocalReqdProp) != NULL) &&
    ...
    if ((ContNo == 0) && (MExpr->GetCounter() == 0)) {
      PTRACE("New winner %s is not cheaper than old winner, so deleted !!!",
             MExpr->Dump().c_str());
  ...
  if (LocalWinner->GetMPlan() && // If there is already a non-null local winner
      CostSoFar >=
          *(LocalWinner->GetCost()) // and current expression is more expensive
  )
...
TerminateThisTask:
  ...
  if (Last) {
    ...
    os.Format("Terminate: replaced winner with %s, %s, %s\r\n",
              LocalReqdProp->Dump().c_str(),
              TempME ? TempME->Dump().c_str() : " NULL ",
...
APPLY_RULE::APPLY_RULE(RULE *rule, M_EXPR *mexpr, bool explore, int ContextID,
                       int parent_task_no, bool last, COST *bound)
    : TASK(ContextID, parent_task_no), Rule(rule), MExpr(mexpr),
      explore(explore), Last(last), EpsBound(bound) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_APPLY_RULE].New();
...
void APPLY_RULE::perform() {
  ...
  if (Context->is_done()) {
    PTRACE("Context: %s is done", Context->GetPhysProp()->Dump().c_str());
    delete this;
    return;
  }
#else
...
#ifdef UNIQ
  // Check again to see that the rule has not been fired since this was put on
  // the stack
  if (!(MExpr->can_fire(Rule->get_index()))) {
    // tasks must destroy themselves
    PTRACE("Rejected rule %d ", Rule->get_index());
  ...
  if (!ForGlobalEpsPruning)
    OptStat->FiredRule++; // Count invocations of this task
  ...
  for (; bindery->advance(); delete before) {
    ...
    if (!Rule->condition(before, MExpr, ContextID)) {
      PTRACE("Binding FAILS condition function, expr: %s",
             MExpr->Dump().c_str());
      continue; // try to find another binding
    }
    PTRACE("Binding SATISFIES condition function.  Mexpr: %s",
           MExpr->Dump().c_str());
    ...
    if (explore) // optimizer is exploring, the new mexpr must be logical expr
    {
      ...
      PTRACE("new task to explore new expression, \
				pushing O_EXPR exploring expr: %s",
             NewMExpr->Dump().c_str());
      if (GlobepsPruning) {
        COST *eps_bound = new COST(*EpsBound);
        PTasks.push(
            new O_EXPR(NewMExpr, true, ContextID, TaskNo, Flag, eps_bound));
      } else
    ...
    else // optimizer is optimizing
    {
      ...
      if (NewMExpr->GetOp()->is_logical()) {
        ...
        if (GlobepsPruning) {
          COST *eps_bound = new COST(*EpsBound);
          PTasks.push(
              new O_EXPR(NewMExpr, false, ContextID, TaskNo, Flag, eps_bound));
        } else
        ...
        if (GlobepsPruning) {
          COST *eps_bound = new COST(*EpsBound);
          PTasks.push(
              new O_INPUTS(NewMExpr, ContextID, TaskNo, Flag, eps_bound));
        } else {
          int contextNo = 0;
          int j = 0;
#ifdef IRPROP
          ...
          for (int i = j; i < M_WINNER::mc[GrpNo]->GetWide(); i++) {
            if (i != ContextID) {
              PTasks.push(
                  new O_INPUTS(NewMExpr, i, TaskNo, Flag, NULL, contextNo++));
            }
          }
          if (!(j == 1 && ContextID == 0))
  ...
  while (--num_afters >= 0) {
    ...
    if (explore) // optimizer is exploring, the new mexpr must be logical expr
    {
      ...
      PTasks.push(
          new O_EXPR(Afters[num_afters].m_expr, true, ContextID, TaskNo, Flag));
    ...
    else // optimizer is optimizing
    {
      ...
      if (Afters[num_afters].m_expr->GetOp()->is_logical()) {
        PTRACE("new task to optimize new expression,pushing O_EXPR, expr: %s",
               Afters[num_afters].m_expr->Dump().c_str());
        PTasks.push(new O_EXPR(Afters[num_afters].m_expr, false, ContextID,
                               TaskNo, Flag));
      } // further transformations to optimize new expr
      else {
        // for a physical operator, optimize the inputs
        ...
        PTRACE("new task to optimize inputs,pushing O_INPUT, epxr: %s",
               Afters[num_afters].m_expr->Dump().c_str());
        PTasks.push(
            new O_INPUTS(Afters[num_afters].m_expr, ContextID, TaskNo, Flag));
      } // for a physical operator, optimize the inputs
...
CString APPLY_RULE::Dump() {
  ...
  os.Format("APPLY_RULE rule: %s, mexpr %s, parent task %d",
            Rule->Dump().c_str(), MExpr->Dump().c_str(), ParentTaskNo);
