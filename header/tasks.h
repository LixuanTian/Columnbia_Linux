...
class PTASKS; // Pending tasks - some structure which contains all tasks waiting
              // to execute
class O_GROUP; // Optimize a GROUP - find the cheapest plan in the group
               // satisfying a context
...
class O_INPUTS; // Optimize inputs - determine if this expression satisfies the
                // current context
...
/*
     ...
     Second, hinted at by Goetz, applies only when there are multiple
   expressions in the group, e.g. after exploring.  Here we can consider all
   rules on all expressions, and fire them in order of their promise.  Promise
   here may include a lower bound estimate for the expressions.
...
private:
  ...
  COST *EpsBound; // if global eps pruning is on, this is the eps bound for eps
                  // pruning
                  // else it is zero
public:
...
/*
   ...
   For example, the associativity rule requires that one input contain a join.
   The E_GROUP task explores a group by creating all target operators that could
...
public:
  ...
  E_GROUP(GRP_ID GrpID, int ContextID, int parent_task_no, bool last = false,
          COST *epsbound = NULL);
  // ##ModelId=3B0C085D0306
  ~E_GROUP() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_E_GROUP].Delete();
    if (EpsBound)
      delete EpsBound;
  };
...
/*
   ...
   when it is used for exploring, fire only transformation rules to prepare for
   a transform
*/
...
private:
  ...
  const bool
      explore; // if this task is for exploring  Should not happen - see E_GROUP
  // ##ModelId=3B0C085D03D8
  bool Last; // if this task is the last task for the group
  // ##ModelId=3B0C085E000E
  COST *EpsBound; // if global eps pruning is on, this is the eps bound of this
                  // task
                  // else it is zero

public:
  // ##ModelId=3B0C085E0018
  O_EXPR(M_EXPR *mexpr, bool explore, int ContextID, int parent_task_no,
         bool last = false, COST *epsbound = NULL);
...
public:
  ...
  O_INPUTS(M_EXPR *MExpr, int ContextID, int ParentTaskNo, bool last = false,
           COST *epsbound = NULL, int ContNo = 0);
...
public:
  ...
  APPLY_RULE(RULE *rule, M_EXPR *mexpr, bool explore, int ContextID,
             int parent_task_no, bool last = false, COST *epsbound = NULL);
