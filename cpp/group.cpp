...
GROUP::GROUP(M_EXPR *MExpr)
    : GroupID(MExpr->GetGrpID()), FirstLogMExpr(MExpr), LastLogMExpr(MExpr),
      FirstPhysMExpr(NULL), LastPhysMExpr(NULL) {
  if (!ForGlobalEpsPruning)
    ClassStat[C_GROUP].New();
...
void GROUP::ShrinkSubGroup() {
  for (M_EXPR *MExpr = FirstLogMExpr; MExpr != NULL;
...
CString GROUP::Dump() {
  ...
  for (int i = 0; i < Size; i++) {
    ...
    os += ", ";
    os +=
        (Winners[i]->GetMPlan() ? Winners[i]->GetMPlan()->Dump() : "NULL Plan");
    os += ", ";
...
void GROUP::FastDump() {
  ...
  PHYS_PROP *PhysProp;
  if (!Size)
    TRACE_FILE("%s", "\tNo Winners\r\n");
  for (int i = 0; i < Size; i++) {
    PhysProp = Winners[i]->GetPhysProp();
    TRACE_FILE("\t%s", PhysProp->Dump().c_str());
    TRACE_FILE(", %s",
               (Winners[i]->GetMPlan() ? Winners[i]->GetMPlan()->Dump().c_str()
                                       : "NULL"));
    TRACE_FILE(
        ", %s",
        (Winners[i]->GetCost() ? Winners[i]->GetCost()->Dump().c_str() : "-1"));
