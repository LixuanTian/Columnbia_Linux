...
public:
  ...
  // add assert to the following virtual functions, make sure the subclasses
  // define them
  ...
  // For example, the operator SELECT has arity 2 (one bulk input, one
  // predicate) and
  // its name is SELECT.  These are not stored as member data since static does
  // not inherit and we don't want one in every object.
  ...
  virtual bool operator==(OP *other) {
    return (GetNameId() == other->GetNameId());
  };

  // Used to compute the hash value of an mexpr.  Used only for logical
  // operators, so we make it abort everywhere else.
  ...
  virtual COST *FindLocalCost(LOG_PROP *LocalLogProp,
                              LOG_PROP **InputLogProp) = 0;
}; // class OP
...
public:
  ...
  virtual COST *
  FindLocalCost(LOG_PROP *LocalLogProp, // uses primarily the card of the Group
...
virtual int opt_combs() */
  ...
  // add assert to the following functions,
  // make sure these methods of PHYS_OP never called(log_prop is not passed by
  // phys_op)
...
/*
    ============================================================
        ITEM OPERATORS - ACT ON INDIVIDUAL OBJECTS - USED IN PREDICATES - class
   ITEM_OP
    ============================================================
*/
...
// AGG_OP may also involve a more complex expression, e.g., with more than one
// function. It is part of the logical operator AGG_LIST and physical op
// HGROUP_LIST
...
public:
  ...
  AGG_OP(CString range_var, int *atts, int size)
    ...
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_AGG_OP].New();
  };

  // ##ModelId=3B0C08730159
  AGG_OP(AGG_OP &Op)
      : RangeVar(Op.RangeVar), AttsSize(Op.AttsSize),
        Atts(CopyArray(Op.Atts, Op.AttsSize)) {
    if (TraceOn && !ForGlobalEpsPruning)
  ...
  CString Dump() {
    ...
    if (AttsSize == 0)
      ...
      temp.Format("%s", GetAttName(Atts[AttsSize - 1]).c_str());
    os += temp;
