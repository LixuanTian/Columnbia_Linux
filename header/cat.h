...
class CAT {
private: // Each array maps an integer into the elements of the array at that
         // integer location, i.e. maps i to array[i].
  ...
  CArray<BIT_IND_PROP *, BIT_IND_PROP *> BitIndProps; // Properties of
                                                      // BitIndexes
...
public:
  ...
  // Each of the following functions retrieves data from one of the private
  // arrays above. If the index input is not within range it returns NULL.
  ...
  // If Attribute or Collection are new, add them to AttProps, AttTable,
  // AttNames, resp. Add AttProp to AttProps table, Attribute to Attnames
  ...
  void AddBitIndex(CString RelName, CString BitIndexName,
...
private: // functions for parsing the catalog input file
  ...
  void parseAttribute(char *p, CString &AttrName, ATTR *Attribute,
                      DOM_TYPE &domain);

  // get index prop. from line buf
  // ##ModelId=3B0C08780106
  void parseIndex(char *p, CString RelName, CString &IndexName,
                  IND_PROP *Index);

  // get bit index prop. from line buf
  // ##ModelId=3B0C08780138
  void parseBitIndex(char *p, CString RelName, CString &BitIndexName,
