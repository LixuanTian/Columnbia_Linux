...
CAT::CAT(CString filename) {
  ...
  for (;;) {
    ...
    if (p == strstr(p, KEYWORD_KEYORDER)) {
      ...
      while (*p != '(')
        p++; // skip the char before '('
      if (*p != '(')
        OUTPUT_ERROR("can't find '(' when parse KEYS_ORDER!");
      p++;
    ...
    if (p == strstr(p, KEYWORD_NUMOFATTR)) {
      ...
      for (int i = 0; i < NumberOfAttribute;) {
        ...
        if (feof(fp))
          OUTPUT_ERROR("EOF when reading attribute!");
    ...
    if (p == strstr(p, KEYWORD_NUMOFINDEX)) {
      ...
      for (int i = 0; i < NumberOfIndex;) {
        ...
        if (feof(fp))
          OUTPUT_ERROR("EOF when reading Index info!");
    ...
    if (p == strstr(p, KEYWORD_NUMOFBITINDEX)) {
      ...
      for (int i = 0; i < NumberOfBitIndex;) {
        ...
        if (feof(fp))
          OUTPUT_ERROR("EOF when reading BitIndex info!");
    ...
    if (p == strstr(p, KEYWORD_NUMOFFOREIGNKEY)) {
      ...
      for (int i = 0; i < NumberOfFKey;) {
        ...
        if (feof(fp))
          OUTPUT_ERROR("EOF when reading Foreign Key info!");
  ...
  for (int CollId = 1; CollId < CollProps.GetSize(); CollId++) {
    ...
    if (CollProp->ForeignKeyString.GetSize() > 0) {
      for (int keyNum = 0; keyNum < CollProp->ForeignKeyString.GetSize();
           keyNum++) {
  ...
  for (int BitIndID = 1; BitIndID < BitIndProps.GetSize(); BitIndID++) {
    BitIndProps[BitIndID]->IndexAttr =
        GetAttId(BitIndProps[BitIndID]->IndexAttrString);
  }
...
// Add CollProp for this collection.  If Collection is new, also update
// CollTable
// ##ModelId=3B0C08780016
void CAT::AddColl(CString CollName, COLL_PROP *CollProp) {
  int CollId = GetCollId(CollName);
  if (CollId >= CollProps.GetSize())
    CollProps.SetSize(CollId + 1);
  CollProps[CollId] = CollProp;
}

// fill tables related to attributes
// If Attribute or Collection are new, add them to AttProps, AttTable, AttNames,
// resp. Add Attr to Attrs table, Attribute to Attnames
// ##ModelId=3B0C08780034
void CAT::AddAttr(CString CollName, CString AttName, ATTR *Attr,
                  DOM_TYPE domain) {
  // If Attribute is new, add it to AttProps.  Add AttProp to AttProps table.
  int AttId = GetAttId(CollName, AttName);
  if (AttId >= Attrs.GetSize())
    Attrs.SetSize(AttId + 1);
  Attr->AttId = AttId;
  Attrs[AttId] = Attr;
  if (AttId >= Domains.GetSize())
    Domains.SetSize(AttId + 1);
...
void CAT::AddIndex(CString CollName, CString IndexName, IND_PROP *IndProp) {
  ...
  if (IndId >= IndProps.GetSize())
    IndProps.SetSize(IndId + 1);
...
// If BitIndex, COllection are new, add them to BitIndProps, BitIndNames,
// respectively. Add BitIndProp, BitIndex to BitIndProps, BitIndNames, resp.
// ##ModelId=3B0C0878008E
void CAT::AddBitIndex(CString CollName, CString BitIndexName,
                      BIT_IND_PROP *BitIndProp) {
  // If BitIndex is new, add it to BitIndProps.  Add BitIndProp to BitIndProps
  int BitIndId = GetBitIndId(CollName, BitIndexName);
  if (BitIndId >= BitIndProps.GetSize())
    BitIndProps.SetSize(BitIndId + 1);
...
void CAT::GetKey(char *p, KEYS_SET *Keys) {
  while (*p != '(')
    p++; // skip the char before '('
  if (*p != '(')
    OUTPUT_ERROR("can't find '(' when parse KEYS_SET!");
  ...
  while (*p != ')') {
    ...
    if (*p == '.') {
      ...
      // else
      OUTPUT_ERROR(
          "format error when parse KEYS_SET! suppose: (X.X) or (X.X,Y.Y)");
    }
...
// fill out attribute, except for AttId, which is filled out when adding
// attribute
// ##ModelId=3B0C087800DE
void CAT::parseAttribute(char *p, CString &AttrName, ATTR *Attribute,
                         DOM_TYPE &domain) {
  p = SkipSpace(p);
...
void CAT::parseIndex(char *p, CString CollName, CString &IndexName,
                     IND_PROP *Index) {
  p = SkipSpace(p);
  parseString(p); // get the Name
  IndexName = p;
...
void CAT::parseBitIndex(char *p, CString CollName, CString &BitIndexName,
                        BIT_IND_PROP *BitIndex) {
  p = SkipSpace(p);
  parseString(p); // get the Name
  BitIndexName = p;
  p += strlen(p) + 1;
  ...
  while (*p != '(')
    p++; // skip the char before '('
  if (*p != '(')
    OUTPUT_ERROR("can't find '(' when parse bit index predicate!");
  int LeftNum = 0;
  int i = 0;
  while (*p) {
    while (*p != ' ' && *p != '\t' && *p != '\n') {
