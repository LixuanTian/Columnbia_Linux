...
// Physical Dummy Operator.  Just to give the dummy operator a physical
// counterpart.
...
public:
  ...
  ~LOOPS_INDEX_JOIN() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_LOOPS_INDEX_JOIN].Delete();
    delete[] lattrs;
    delete[] rattrs;
  };
...
public:
  ...
  HASH_DUPLICATES() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_HASH_DUPLICATES].New();
  };
  ...
  ~HASH_DUPLICATES() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_HASH_DUPLICATES].Delete();
  };
...
public:
  ...
  ~HGROUP_LIST() {
    ...
    delete[] GbyAtts;
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_HGROUP_LIST].Delete();
  };
...
public:
  ...
  ~INDEXED_FILTER() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_INDEXED_FILTER].Delete();
  };
