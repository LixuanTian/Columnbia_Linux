...
public:
  ...
  inline bool operator==(OP *other) {
    return (other->GetNameId() == GetNameId() &&
            EqualArray(((EQJOIN *)other)->lattrs, lattrs,
                       size) && // arguments are equal
...
DUMMY
============================================================
For Paul Benninghoff's experiment.  Used to determine if optimizing two queries
at once can result in significant savings.  DUMMY has two inputs, no transforms,
one implementation, namely PDUMMY.
*/
...
/* ============================================================
   SELECT
   ============================================================
   This is the standard select operator: only input objects satisfying a
   predicate will be output.  However, we model the predicate of the operator as
   its second input.  Cascades modeled predicates as inputs in general, so that
   their structure could be handled with the Cascades transformation mechanism.
*/
...
public:
  ...
  inline int GetArity() {
    return (1);
  }; // Only input is the stream of bytes.  What to project is an argument,
     // pattrs
  ...
  inline bool operator==(OP *other) {
    return (other->GetNameId() == GetNameId() &&
            EqualArray(((PROJECT *)other)->attrs, attrs,
                       size)); // arguments are equal
  };
...
public:
  ...
  ~RM_DUPLICATES() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_RM_DUPLICATES].Delete();
  };
...
public:
  ...
  ~AGG_LIST() {
    delete[] GbyAtts;
    for (int i = 0; i < AggOps->GetSize(); i++)
      delete (*AggOps)[i];
    delete AggOps;
    delete[] FlattenedAtts;
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_AGG_LIST].Delete();
  };
...
public:
  ...
  inline bool operator==(OP *other) {
    return (other->GetNameId() == GetNameId() &&
            EqualArray(((FUNC_OP *)other)->Atts, Atts,
                       AttsSize) && // arguments are equal
