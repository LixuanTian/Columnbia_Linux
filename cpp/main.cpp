...
void Optimizer() {
...
#ifdef LINUX_PORT
  if (CatFile == "catalog")
    CatFile = AppDir + "/catalog";
  if (CMFile == "cm")
    CMFile = AppDir + "/cm";
  if (RSFile == "ruleset")
    RSFile = AppDir + "/ruleset";
#else
  if (CatFile == "catalog")
    CatFile = AppDir + "\\catalog";
  if (CMFile == "cm")
...
#ifdef LINUX_PORT
  OutputFile.Open((AppDir + "/colout.txt").c_str(),
                  CFile::modeCreate | CFile::modeWrite);
  OutputCOVE.Open((AppDir + "/script.cove").c_str(),
                  CFile::modeCreate | CFile::modeWrite);
#else
  ...
  // SQueryFile, BQueryFile have been set to the Single or Batch Query File
  // chosen in
  //   the option dialog.  The CString QueryFile will be set to the name of a
  //   file containing the query currently being optimized.  In the single query
  //   case that will be SQueryFile, in the Batch Query case it will be
  //   "tempquery", which will contain the current batch query, copied from
  //   BQueryFile.  The default case is handled separately, with files named
  //   query and bquery.
...
#ifdef LINUX_PORT
    if (SQueryFile == "query")
      QueryFile = AppDir + "/query"; // default case
#else
    if (SQueryFile == "query")
      QueryFile = AppDir + "\\query"; // default case
#endif
      ...
      QueryFile =
          SQueryFile; // value entered in option dialog; should check it exists.
    NumQuery = 1;
  } else if (RadioVal == 0) // Batch Query case
  {
    // Open BQueryFile
#ifdef LINUX_PORT
    if (BQueryFile == "bquery")
      BQueryFile = AppDir + "/bquery"; // default case
#else
    if (BQueryFile == "bquery")
      BQueryFile = AppDir + "\\bquery"; // default case
#endif
  ...
  do {
    ...
    if (RadioVal == 0) // Batch Query case
    {
      ...
      for (;;) {
        ...
        if (p == strstr(p, KEYWORD_NUMOFQRY)) {
          ...
          OUTPUT_ERROR(
              "the first line in the batch file should be number of queries");
      }
    }
    ...
    for (int q = 0; q < NumQuery; q++) {
      ...
      for (double ii = 0; ii <= GLOBAL_EPS * 10; ii++) {
        ...
        PTRACE("---1--- memory statistics before optimization: %s",
               DumpStatistics().c_str());
        ...
        Ssp->optimize(); // Later add an input condition so we can handle ORDER
                         // BY
        ...
        if (!SingleLineBatch) {
          OUTPUT("Optimization elapsed time:\t\t%s", tmpbuf.c_str());
        ...
        */
        ...
        PTRACE("---2--- memory statistics after optimization: %s",
               DumpStatistics().c_str());
        if (TraceFinalSSP) {
          Ssp->FastDump();
        } else {
        ...
        if (!PiggyBack)
          delete Ssp;
        PTRACE("---3--- memory statistics after freeing searching space: %s",
               DumpStatistics().c_str());
