...
CString RULE_SET::DumpStats() {
  ...
  os += temp;
  for (int i = 0; i < RuleCount; i++) {
    temp.Format("%d\t%d\t%d\t%d\t%s\r\n", i, TopMatch[i], Bindings[i],
                Conditions[i], rule_set[i]->GetName());
...
BINDERY::BINDERY(GRP_ID group_no, EXPR *original)
    : state(start), group_no(group_no), cur_expr(NULL), original(original),
      input(NULL), one_expr(FALSE) // try all expressions within this group
{
  ASSERT(original);

  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_BINDERY].New();
} // BINDERY::BINDERY
...
BINDERY::~BINDERY() {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_BINDERY].Delete();

  if (input != NULL) {
    for (int i = 0; i < original->GetOp()->GetArity(); i++)
      delete input[i];
    delete[] input;
  }
...
EXPR *BINDERY::extract_expr() {
  ...
  if (patt_op->is_leaf()) {
    result = new EXPR(new LEAF_OP(((LEAF_OP *)patt_op)->GetIndex(), group_no));
  }    // create leaf marked with group index
...
bool BINDERY::advance() {
  ...
  {                                // Search entire group for bindings
    cur_expr =
        Ssp->GetGroup(group_no)->GetFirstLogMExpr(); // get the first mexpr
  }
    ...
    case start:
      ...
      if (arity == 0) // only the Operator, matched
      {
        ...
        for (input_no = 0; input_no < arity; input_no++) {
          input[input_no] = new BINDERY(cur_expr->GetInput(input_no),
        ...
        // found one more binding
        {
          ...
          for (int other_input_no = input_no; ++other_input_no < arity;) {
            ...
            input[other_input_no] =
                (new BINDERY(cur_expr->GetInput(other_input_no),
...
GET_TO_FILE_SCAN::GET_TO_FILE_SCAN()
    : RULE("GET_TO_FILE_SCAN", 0, new EXPR(new GET(0)),
...
EQ_TO_LOOPS::EQ_TO_LOOPS()
    : RULE("EQJOIN->LOOPS_JOIN", 2,
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))),
           new EXPR(new LOOPS_JOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1)))) {
  // set rule index
...
EXPR *EQ_TO_LOOPS_INDEX::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  result =
      new EXPR(new LOOPS_INDEX_JOIN(lattrs, rattrs, size, g->GetCollection()),
               new EXPR(*(before->GetInput(0))));
...
bool EQ_TO_LOOPS_INDEX::condition(EXPR *before, M_EXPR *mexpr, int ContextID) {
  ...
  for (int i = 0; i < Indices->GetSize(); i++)
    ...
    if (size == 1 &&
        Cat->GetIndProp((*Indices)[i])->Keys->ContainKey(rattrs[0]))
      return (true);
...
EQ_TO_MERGE::EQ_TO_MERGE()
    : RULE("EQJOIN -> MERGE_JOIN", 2,
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))),
           new EXPR(new MERGE_JOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1)))) {
  // set rule index
...
EQ_TO_HASH::EQ_TO_HASH()
    : RULE("EQJOIN->HASH_JOIN", 2,
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))),
           new EXPR(new HASH_JOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1)))) {
  // set rule index
...
EQJOIN_COMMUTE::EQJOIN_COMMUTE()
    : RULE("EQJOIN_COMMUTE", 2,
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))),
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(1)),
                    new EXPR(new LEAF_OP(0)))) {

  // set rule mask and index
  set_index(R_EQJOIN_COMMUTE);
  set_mask(1 << R_EQJOIN_COMMUTE | 1 << R_EQJOIN_LTOR | 1 << R_EQJOIN_RTOL |
           1 << R_EXCHANGE);
...
EQJOIN_LTOR::EQJOIN_LTOR()
    : RULE("EQJOIN_LTOR", 3,
           ...
           new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(1)),
                             new EXPR(new LEAF_OP(2)))) // substitute
      )
...
EXPR *EQJOIN_RTOL::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  EXPR *result =
      new EXPR(new EQJOIN(nlattrs2, nrattrs2, nsize2), // args reversed
               new EXPR(new EQJOIN(nlattrs1, nrattrs1, nsize1),
                        new EXPR(*(before->GetInput(0))),
...
SELECT_TO_FILTER::SELECT_TO_FILTER()
    : RULE("SELECT -> FILTER", 2,
           ...
           new EXPR(new FILTER, new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1)))) {
  // set rule index
...
EXPR *SELECT_TO_FILTER::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  result = new EXPR(new FILTER, new EXPR(*(before->GetInput(0))),
                    new EXPR(*(before->GetInput(1))));
...
P_TO_PP::P_TO_PP()
    : RULE("PROJECT -> P_PROJECT", 1,
           new EXPR(new PROJECT(0, 0), new EXPR(new LEAF_OP(0))),
           new EXPR(new P_PROJECT(0, 0), new EXPR(new LEAF_OP(0)))) {
  // set rule index
  set_index(R_P_TO_PP);
} // P_TO_PP::P_TO_PP
...
EXPR *P_TO_PP::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  for (int i = 0; i < size; i++)
    assert(As_schema->InSchema(attrs[i]));

#endif

  // create transformed expression
  result =
      new EXPR(new P_PROJECT(attrs, size), new EXPR(*(before->GetInput(0))));
...
SORT_RULE::SORT_RULE()
    : RULE("SORT enforcer", 1, new EXPR(new LEAF_OP(0)),
...
EXPR *SORT_RULE::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  // create transformed expression
  EXPR *result = new EXPR(new QSORT(), new EXPR(*before));
  return (result);
}
...
RM_TO_HASH_DUPLICATES::RM_TO_HASH_DUPLICATES()
    : RULE("RM_DUPLICATES  -> HASH_DUPLICATES", 1,
           ...
           new EXPR(new HASH_DUPLICATES(), new EXPR(new LEAF_OP(0)))) {
  // set rule index
  set_index(R_RM_TO_HASH_DUPLICATES);
} // RM_TO_HASH_DUPLICATES::RM_TO_HASH_DUPLICATES

// ##ModelId=3B0C086C00F5
EXPR *RM_TO_HASH_DUPLICATES::next_substitute(EXPR *before,
                                             PHYS_PROP *ReqdProp) {
  EXPR *result;

  // create transformed expression
  result = new EXPR(new HASH_DUPLICATES(), new EXPR(*(before->GetInput(0))));
...
AL_TO_HGL::AL_TO_HGL(AGG_OP_ARRAY *list1, AGG_OP_ARRAY *list2)
    : RULE("AGG_LIST  -> HGROUP_LIST", 1,
           ...
           new EXPR(new HGROUP_LIST(0, 0, list2), new EXPR(new LEAF_OP(0)))) {
  // set rule index
...
FO_TO_PFO::FO_TO_PFO()
    : RULE("FUNC_OP  -> P_FUNC_OP", 1,
           ...
           new EXPR(new P_FUNC_OP("", 0, 0), new EXPR(new LEAF_OP(0)))) {
  // set rule index
    ...
    : RULE("AGGREGATE EQJOIN -> JOIN AGGREGATE", 2,
           new EXPR(new AGG_LIST(0, 0, list1),
                    new EXPR(new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
                             new EXPR(new LEAF_OP(1)))),
           new EXPR(
               new EQJOIN(0, 0, 0), new EXPR(new LEAF_OP(0)),
               new EXPR(new AGG_LIST(0, 0, list2), new EXPR(new LEAF_OP(1))))) {
  // set rule index
...
EXPR *AGG_THRU_EQJOIN::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  for (i = 0; i < agg_op->GbySize; i++) {
    if (right_schema->InSchema(gby_atts[i]))
      new_gby_size++;
  }
  new_gby_size += size;
...
/* Conditions:
        ...
        2. The attributes referenced in the EQJOIN predicate are in the group by
   list
...
bool AGG_THRU_EQJOIN::condition(EXPR *before, M_EXPR *mexpr, int ContextID) {
  ...
  if (l_cand_key->IsSubSet(lattrs, size))
    return (true);
...
EQ_TO_BIT::EQ_TO_BIT()
    : RULE("EQJOIN -> BIT_JOIN", 2,
           ...
           new EXPR(new BIT_JOIN(0, 0, 0, 0), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1)))) {
  // set rule index
...
/* Conditions:
      ...
      3. EQJOIN is a foreign key join, with Lattr the foreign key (rattr is the
   candidate key of relation 'collection')
...
bool EQ_TO_BIT::condition(EXPR *before, M_EXPR *mexpr, int ContextID) {
  ...
  if (size == 1 && l_cand_key && BitIndices) {
    ...
    for (int CurrBitIndex = 0; CurrBitIndex < BitIndices->GetSize();
         CurrBitIndex++) {
      int index_attr =
          Cat->GetBitIndProp((*BitIndices)[CurrBitIndex])->IndexAttr;
      KEYS_SET *bit_attrs =
          Cat->GetBitIndProp((*BitIndices)[CurrBitIndex])->BitAttr;
...
SELECT_TO_INDEXED_FILTER::SELECT_TO_INDEXED_FILTER()
    : RULE("SELECT -> INDEXED_FILTER", 1,
           ...
           new EXPR(new INDEXED_FILTER(0), new EXPR(new LEAF_OP(0)))) {
  // set rule index
  set_index(R_SELECT_TO_INDEXED_FILTER);
} // SELECT_TO_INDEXED_FILTER::SELECT_TO_INDEXED_FILTER

// ##ModelId=3B0C086D0100
EXPR *SELECT_TO_INDEXED_FILTER::next_substitute(EXPR *before,
                                                PHYS_PROP *ReqdProp) {
  EXPR *result;

  // create transformed expression
  result = new EXPR(new INDEXED_FILTER(
                        ((GET *)before->GetInput(0)->GetOp())->GetCollection()),
                    new EXPR(*(before->GetInput(1))));
  return result;
} // GET_TO_FILE_SCAN::next_substitute
...
bool SELECT_TO_INDEXED_FILTER::condition(EXPR *before, M_EXPR *mexpr,
                                         int ContextID) {
  ...
  if (Indices && FreeVar.GetSize() == 1) {
    for (int i = 0; i < Indices->GetSize(); i++) {
      if ((*Cat->GetIndProp((*Indices)[i])->Keys) == FreeVar)
        return (true);
    }
  }
...
EXPR *PROJECT_THRU_SELECT::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  result = new EXPR(
      new PROJECT(CopyArray(top_pattrs, top_size), top_size),
      new EXPR(
          new SELECT,
...
DUMMY_TO_PDUMMY::DUMMY_TO_PDUMMY()
    : RULE("DUMMY->PDUMMY", 2,
           new EXPR(new DUMMY(), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))),
           new EXPR(new PDUMMY(), new EXPR(new LEAF_OP(0)),
                    new EXPR(new LEAF_OP(1))
...
EXPR *DUMMY_TO_PDUMMY::next_substitute(EXPR *before, PHYS_PROP *ReqdProp) {
  ...
  // create transformed expression
  EXPR *result = new EXPR(new PDUMMY(), new EXPR(*(before->GetInput(0))),
                          new EXPR(*(before->GetInput(1))));
  return result;
