...
public:
  ...
  CString Dump() {
    CString os;
    os.Format("ATTR(%s)", GetAttName(AttId).c_str());
    return os;
  }
};
...
public:
  ...
  ~CONST_INT_OP() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_CONST_INT_OP].Delete();
  };
...
public:
  ...
  ~CONST_STR_OP() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_CONST_STR_OP].Delete();
  };
...
public:
  ...
  ~CONST_SET_OP() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_CONST_SET_OP].Delete();
  };
