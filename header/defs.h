...
typedef unsigned int
    BIT_VECTOR; // Used to implement unique rule set.  Note this
// restricts the number of transformational (logical) rules.

extern bool
    ForGlobalEpsPruning; // If true, we are running the optimizer to get an
...
#define OUTPUT_ERROR(text)                                                     \
  {                                                                            \
    ...
    OutputString.Format("%s%s%s%s%s%d%s", "\r\nERROR:", text,                  \
                        ",file:", trimFile.c_str(), ",line:", __LINE__,        \
                        "\r\n");                                               \
    OutputWindow->Print(OutputString.c_str());                                 \
    OutputFile.Write(OutputString.c_str(), OutputString.GetLength());          \
    abort();                                                                   \
...
extern bool
    SingleLineBatch; // Should output of batch queries be one line per query?
...
IRPROP: off == Optimize each group for one property only
                on == Optimize each group for all Interesting Relevant
Properties UNIQ:   on == Use the unique rule set rules USE_MEMORY_MANAGER: Use
memory pooling for selected classes SORT_AFTERS: sort possible moves in order of
estimated cost INFBOUND: When optimizing a group, ignore the initial upper
bound; use infinity instead FIRSTPLAN: trace when the first complete plan is
costed _COSTS_ Prints the cost of each mexpr as it is costed, in the output
window _TABLE_: prints one summary line for each optimization, using different
epsilons _GEN_LOG: Used to control the generation of logical expressions when
eps pruning is done. REUSE_SIB: An attempt to improve pattern matching by
reusing one side of generated mexprs. NOCART: Do not allow Cartesian products
during optimization. SAFETY: Used within Bill's memory manager.  Higher level of
error checking. CONDPRUNE: Use Group Pruning technique as a condition for firing
some rules
*/
