...
CString ATTR_EXP::Dump() {
  ...
  if (AttsSize == 0)
    os.Format("%s", "Empty set");
  else
    os.Format("%s", GetAttName(Atts[AttsSize - 1]).c_str());
...
LOG_PROP *COMP_OP::FindLogProp(LOG_PROP **input) {
  ...
  if (LeftProp->CuCard == -1 || RightProp->CuCard == -1 ||
      (LeftProp->CuCard != 1 && RightProp->CuCard != 1) || op_code == OP_LIKE) {
    // magic numbers
    switch (op_code) {
    case OP_LIKE:
    ...
    case OP_IN:
      ...
      selectivity = const_value / (attr_cucard);
      break;
    case OP_EQ:
      selectivity = 1 / attr_cucard;
      break;
    case OP_NE:
      selectivity = 1 - 1 / attr_cucard;
      break;
    case OP_LE:
    case OP_LT:
      selectivity = (MIN(attr_max, const_value) - MIN(attr_min, const_value)) /
                    (attr_max - attr_min);
      break;
    case OP_GE:
    case OP_GT:
      selectivity = (MAX(attr_max, const_value) - MAX(attr_min, const_value)) /
                    (attr_max - attr_min);
      break;
    };
  }
