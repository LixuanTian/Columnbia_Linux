...
/* for cucard pruning, calculate the minimun cost of fetching cucard tuples from
   disc FetchingCost = Fetch() * sum(cucard(Ai) i = 1, ..., n) // from leaf
   fetches For each Ai which has no index on a join (interesting) order, replace
...
PHYS_PROP *FILE_SCAN::FindPhysProp(PHYS_PROP **input_phys_props) {
  ...
  if (CollProp->Order == any) {
    ...
    PHYS_PROP *phys_prop =
        new PHYS_PROP(new KEYS_SET(*(CollProp->Keys)), CollProp->Order);
...
COST *FILE_SCAN::FindLocalCost(LOG_PROP *LocalLogProp,
  ...
  COST *Result = new COST(
      ceil(Card * Width) * (Cm->cpu_read() + // cpu cost of reading from disk
                            Cm->io())        // i/o cost of reading from disk
  );
  return (Result);
}
...
LOOPS_JOIN::LOOPS_JOIN(LOOPS_JOIN &Op)
    : lattrs(CopyArray(Op.lattrs, Op.size)),
      rattrs(CopyArray(Op.rattrs, Op.size)), size(Op.size) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_LOOPS_JOIN].New();
#ifdef _DEBUG
  name = GetName();
...
COST *LOOPS_JOIN::FindLocalCost(LOG_PROP *LocalLogProp,
                                LOG_PROP **InputLogProp) {
  ...
  COST *result =
      new COST(LeftCard * RightCard * Cm->cpu_pred() // cpu cost of predicates
...
PHYS_PROP *LOOPS_JOIN::InputReqdProp(PHYS_PROP *PhysProp,
                                     LOG_PROP *InputLogProp, int InputNo,
                                     bool &possible) {
...
LOOPS_INDEX_JOIN::LOOPS_INDEX_JOIN(int *lattrs, int *rattrs, int size,
                                   int CollId)
    : lattrs(lattrs), rattrs(rattrs), size(size), CollId(CollId) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_LOOPS_INDEX_JOIN].New();
#ifdef _DEBUG
  name = GetName();
#endif
...
LOOPS_INDEX_JOIN::LOOPS_INDEX_JOIN(LOOPS_INDEX_JOIN &Op)
    : lattrs(CopyArray(Op.lattrs, Op.size)),
      rattrs(CopyArray(Op.rattrs, Op.size)), size(Op.size), CollId(Op.CollId) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_LOOPS_INDEX_JOIN].New();
#ifdef _DEBUG
  name = GetName();
...
COST *LOOPS_INDEX_JOIN::FindLocalCost(LOG_PROP *LocalLogProp,
                                      LOG_PROP **InputLogProp) {
  float LeftCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
...
PHYS_PROP *LOOPS_INDEX_JOIN::InputReqdProp(PHYS_PROP *PhysProp,
                                           LOG_PROP *InputLogProp, int InputNo,
                                           bool &possible) {
...
COST *MERGE_JOIN::FindLocalCost(LOG_PROP *LocalLogProp,
                                LOG_PROP **InputLogProp) {
  float LeftCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
  float RightCard = ((LOG_COLL_PROP *)InputLogProp[1])->Card;
...
PHYS_PROP *MERGE_JOIN::InputReqdProp(PHYS_PROP *PhysProp,
                                     LOG_PROP *InputLogProp, int InputNo,
                                     bool &possible) {
...
PHYS_PROP *MERGE_JOIN::FindPhysProp(PHYS_PROP **input_phys_props) {
  ...
  PHYS_PROP *result =
      new PHYS_PROP(Keys, sorted); // sorted on lattrs, and rattrs
  // the KeyOrder is ascending for all keys
...
HASH join
=========
       Like Merge join, but inputs can have any property.  Operator constructs a
hash table.
*/
...
COST *HASH_JOIN::FindLocalCost(LOG_PROP *LocalLogProp,
                               LOG_PROP **InputLogProp) {
  float LeftCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
  float RightCard = ((LOG_COLL_PROP *)InputLogProp[1])->Card;
  ...
  COST *result =
      new COST(RightCard * Cm->hash_cost()   // cpu cost of building hash table
...
COST *FILTER::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp) {
  ...
  COST *result = new COST(InputCard * Cm->cpu_pred() // cpu cost of predicates
                          + OutputCard * Cm->touch_copy() // cpu cost of copying
                                                          // result no i/o cost
  );
...
P_PROJECT::P_PROJECT(P_PROJECT &Op)
    : attrs(CopyArray(Op.attrs, Op.size)), size(Op.size) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_P_PROJECT].New();
#ifdef _DEBUG
  name = GetName();
...
COST *P_PROJECT::FindLocalCost(LOG_PROP *LocalLogProp,
                               LOG_PROP **InputLogProp) {
  float InputCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
  ...
  // Need to have a cost for 0 tuples case	+ 1 ??
  COST *result = new COST(InputCard * Cm->touch_copy() // cpu cost of copying
                                                       // result no i/o cost
  );
...
CString QSORT::Dump() { return GetName(); } // QSORT::Dump
...
COST *QSORT::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp) {
  ...
  COST *result =
      new COST(2 * card * log(card) / log(2.0) // number of comparison and move
...
COST *HASH_DUPLICATES::FindLocalCost(LOG_PROP *LocalLogProp,
                                     LOG_PROP **InputLogProp) {
  float InputCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
...
PHYS_PROP *HASH_DUPLICATES::InputReqdProp(PHYS_PROP *PhysProp,
                                          LOG_PROP *InputLogProp, int InputNo,
                                          bool &possible) {
...
CString HASH_DUPLICATES::Dump() { return GetName(); } // HASH_DUPLICATES::Dump
...
COST *HGROUP_LIST::FindLocalCost(LOG_PROP *LocalLogProp,
                                 LOG_PROP **InputLogProp) {
  float InputCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
  float OutputCard = ((LOG_COLL_PROP *)LocalLogProp)->Card;
  ...
  COST *result = new COST(
      InputCard * (Cm->hash_cost() // cost of hashing
                   + Cm->cpu_apply() *
                         (AggOps->GetSize())) // apply the aggregate operation
...
PHYS_PROP *HGROUP_LIST::InputReqdProp(PHYS_PROP *PhysProp,
                                      LOG_PROP *InputLogProp, int InputNo,
                                      bool &possible) {
  assert(InputNo == 0); // one input
...
PHYS_PROP *HGROUP_LIST::FindPhysProp(PHYS_PROP **input_phys_props) {
  ...
  PHYS_PROP *result =
      new PHYS_PROP(Keys, sorted); // sorted on group_by attributes
  for (int i = 0; i < GbySize; i++)
    result->KeyOrder.Add(ascending);
...
CString HGROUP_LIST::Dump() {
  ...
  if (GbySize > 0)
    temp.Format("%s )", GetAttName(GbyAtts[i]).c_str());
  else
...
COST *P_FUNC_OP::FindLocalCost(LOG_PROP *LocalLogProp,
                               LOG_PROP **InputLogProp) {
  float InputCard = ((LOG_COLL_PROP *)InputLogProp[0])->Card;
  float OutputCard = ((LOG_COLL_PROP *)LocalLogProp)->Card;
...
INDEXED_FILTER ::INDEXED_FILTER(const int fileId) : FileId(fileId) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_INDEXED_FILTER].New();
#ifdef _DEBUG
  name = GetName() + GetCollName(FileId);
#endif
};
...
COST *INDEXED_FILTER::FindLocalCost(LOG_PROP *LocalLogProp,
                                    LOG_PROP **InputLogProp) {
  ...
  // for model D, redicate must be on a single attribute to use this physical
  // operator.
  ...
  if (Clustered) {
    ...
    data_cost = ceil(OutputCard * Width) *
                (Cm->cpu_read() // cpu cost of reading data from disk
    ...
    index_cost =
        ceil(OutputCard / Cm->index_bf()) * (Cm->cpu_read() + Cm->io());
    // read all data blocks or read data for every index entry found
    data_cost =
        MIN(ceil(InputCard * Width), OutputCard) * (Cm->cpu_read() + Cm->io());
  }
...
PHYS_PROP *INDEXED_FILTER::InputReqdProp(PHYS_PROP *PhysProp,
                                         LOG_PROP *InputLogProp, int InputNo,
                                         bool &possible) {
  assert(InputNo == 0); // one input
  possible = true;
