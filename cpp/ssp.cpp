...
CString SSP::DumpHashTable() {
  ...
  for (int i = 0; i < HtblSize; i++) {
    for (M_EXPR *mexpr = HashTbl[i]; mexpr != NULL;
         mexpr = mexpr->GetNextHash(), total++)
      os += mexpr->Dump();
    if (HashTbl[i])
      os += "\r\n";
  }
...
void SSP::Shrink() {
  for (int i = InitGroupNum; i < Groups.GetSize(); i++)
    ShrinkGroup(i);
}
...
void SSP::ShrinkGroup(GRP_ID group_no) {
  ...
  if (!Group->is_optimized() && !Group->is_explored())
    return; // may be pruned
  ...
  while (mexpr != NULL) {
    ...
    for (p = HashTbl[hashval], prev = NULL; p != mexpr;
...
M_EXPR *SSP::FindDup(M_EXPR &MExpr) {
  ...
  for (M_EXPR *old = prev; old != NULL;
       prev = old, old = old->GetNextHash(), BucketSize++) {
    ...
    if (!(*(old->GetOp()) == MExpr.GetOp())) {
      PTRACE2("Different at Operator. %s : %s", old->Dump().c_str(),
              MExpr.Dump().c_str());
      goto not_a_duplicate;
    }
  ...
  if (!ForGlobalEpsPruning) {
    if (OptStat->MaxBucket < BucketSize)
      OptStat->MaxBucket = BucketSize;
  }
...
M_EXPR *SSP::CopyIn(EXPR *Expr, GRP_ID &GrpID) {
  ...
  if (MExpr->GetOp()->is_logical()) {
    ...
    {
      if (!ForGlobalEpsPruning)
        OptStat->DupMExpr++; // calculate dup mexpr
      PTRACE("duplicate mexpr : %s", MExpr->Dump().c_str());
  ...
  if (GrpID == NEW_GRPID) {
    ...
    if (GrpID >= Groups.GetSize())
      Groups.SetSize(GrpID + 1);
    Groups[GrpID] = Group;

#ifdef IRPROP

    // For the topmost group and for the groups containing the item operator and
    // constant operator, set the only physical property as any and bound INF
    if (!win || GrpID == 0 || ((MExpr->GetOp())->is_const()) ||
        ((MExpr->GetOp())->is_item())) {
      M_WINNER *MWin = new M_WINNER(1);
      if (GrpID >= M_WINNER::mc.GetSize())
        M_WINNER::mc.SetSize(GrpID + 1);
      M_WINNER::mc[GrpID] = MWin;
    } else {
      KEYS_SET *tmpKeySet;

      // get the relevant attributes from the schema for this group
      tmpKeySet =
          (((LOG_COLL_PROP *)(Group->get_log_prop()))->Schema)->AttrStore();
...
void SSP::CopyOut(GRP_ID GrpID, PHYS_PROP *PhysProp, int tabs) {
  ...
  // It's an item group
    ...
    os.Format("%s", WinnerOp->Dump().c_str());
  ...
  // it's a normal group
    ...
    // Now extract the operator from the expression and write it to the output
    // string
...
#ifndef _TABLE_
    OUTPUT("%s", os.c_str());
    if (SingleLineBatch) // In this case we want only the total cost of the
                         // Winner
    {
      OUTPUT("%s", "\r\n");
      return;
    ...
    for (int i = 0; i < Arity; i++) {
      ...
      ReqProp = ((PHYS_OP *)WinnerOp)
                    ->InputReqdProp(
                        PhysProp, Ssp->GetGroup(input_groupno)->get_log_prop(),
                        i, possible);
...
/* bool GROUP::search_circle(CONT * C, bool & moresearch)
    {
            ...
            If (WPlan is non-null) //Cheapest plan costs *WCost;
                    ...
                    if( *CCost <= *WCost)
                            Case (1)
                    else if (*WCost < *CCost) //There might be a plan between
   WCost and CCost Case (4)
*/
...
No winner for this property:  (3)
                                ...
                                WCost >= CCost	WCost < CCost	WCost <= CCost
WCost > CCost
...
bool GROUP::search_circle(CONT *C, bool &moreSearch) {
  ...
  {
    if (*CCost >=
        *WCost) // Real winner; CCost is less of a constraint.  Case (2)
    {
      moreSearch = false;
      return (true);
    } else // search is impossible as winner's cost is more than required
           // context cost (1)
    {
      moreSearch = false;
      return (false);
  ...
  {
    if (*WCost >=
        *CCost) // Previous search failed and CCost is more of a constraint. (1)
    {
      moreSearch = false;
      return (false);
...
void GROUP::NewWinner(PHYS_PROP *ReqdProp, M_EXPR *MExpr, COST *TotalCost,
  ...
  {
    CString os;
    os.Format("NewWin %d \"%s\"%s  { %d %ld \"%s\" %s }\r\n", MExpr->GetGrpID(),
              ReqdProp->Dump().c_str(), TotalCost->Dump().c_str(),
              MExpr->GetGrpID(), (long)MExpr, MExpr->Dump().c_str(),
              done ? "Done" : "Not Done");
...
WINNER::WINNER(M_EXPR *MExpr, PHYS_PROP *PhysProp, COST *Cost, bool done)
    : Cost(Cost), MPlan((MExpr == NULL) ? NULL : (new M_EXPR(*MExpr))),
      PhysProp(PhysProp), Done(done) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_WINNER].New();
};

// ##ModelId=3B0C0868021B
M_WINNER::M_WINNER(int S) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_M_WINNER].New();
