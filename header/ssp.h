...
#define NEW_GRPID_NOWIN                                                        \
  -2 // use by SSP::CopyIn.  Means NEW_GRPID, and this is a
...
private:
  ...
  GRP_ID RootGID; // ID of the oldest, root group.  Set to NEW_GRPID in
                  // SSP""Init then
  // never changes
...
public:
  ...
  inline void set_exploring(bool is_exploring) {
    State.exploring = is_exploring;
  }
  ...
  */
  ...
  void NewWinner(PHYS_PROP *ReqdProp, M_EXPR *MExpr, COST *TotalCost,
                 bool done);
  ...
  void
  DeletePhysMExpr(M_EXPR *PhysMExpr); // delete a physical mexpr from a group

  // ##ModelId=3B0C086700C6
  bool
  CheckWinnerDone(); // check if there is at least one winner done in this group
...
contains the M_EXPR which won a search plus the context (CONT) used in the
search. Done = False, in a winner, means this winner is under construction; its
...
(3) If MPlan is null, and Cost is null:
        ...
        (also should not happen because winners are initialized with context's
costs)
...
/*
    ...
    ============================================================
        A M_WINNER (multiwinner) data structure will, when the group is
   optimized, contain a winner (best plan) for each interesting and relevant
   (i.e., in the schema) property of the group.  The M_WINNER elements are
   initialized with an infinite bound to indicate that no qualifying plan has
   yet been found.

    The idea is then to optimize a group not only for the motivating context but
   also for all the contexts stored in a M_WINNER for that group. In this way,
   we do not have to revisit that group, thus saving CPU time and memory.
*/
...
public:
  ...
  static CArray<M_WINNER *, M_WINNER *> mc;
