...
ub4 lookup2(register ub4 k,       // the key to be hashed
...
ub4 lookup2(register CString k,   // the key to be hashed
            ...
            register ub4 initval) // the previous hash, or an arbitrary value
{
  ...
  while (len >= 12) {
    a += (k[i + 0] + ((ub4)k[i + 1] << 8) + ((ub4)k[i + 2] << 16) +
          ((ub4)k[i + 3] << 24));
    b += (k[i + 4] + ((ub4)k[i + 5] << 8) + ((ub4)k[i + 6] << 16) +
          ((ub4)k[i + 7] << 24));
    c += (k[i + 8] + ((ub4)k[i + 9] << 8) + ((ub4)k[i + 10] << 16) +
          ((ub4)k[i + 11] << 24));
    mix(a, b, c);
    i += 12;
    len -= 12;
  ...
  if (FKeyArray.GetSize() > 0) {
    ...
    for (int i = 0; i < FKeyArray.GetSize(); i++) {
      if (i < FKeyArray.GetSize() - 1)
        ...
        temp.Format("%s%s", (*FKeyArray[i]).Dump().c_str(), "\r\n");
      os += temp;
    }
  }
...
CString ATTR::Dump() {
  CString os;
  os.Format("%s%s%s%s%.0f%s%.0f%s%.0f ", GetAttName(AttId).c_str(),
            " Domain:", DomainToString(Cat->GetDomain(AttId)).c_str(),
  ...
  for (int i = 0; i < size; i++) {
    for (index = 0; index < this->Size; index++) {
      if (attrs[i] == this->Attrs[index]->AttId) {
        ...
        // ATTR * Attr = new ATTR(Attrs[index]->AttId , Attrs[index]->CuCard,
        // -1, -1);
        ...
        for (j = 0; j < new_schema->TableNum; j++)
          if (CollId == new_schema->TableId[j])
            break;

        if (j == new_schema->TableNum) // a new table in the schema
          new_schema->TableId[(new_schema->TableNum)++] =
              CollId; // store the table id

        break;
      }
    }
    if (index == Size)
      assert(false); // project list not in schema
  }
...
SCHEMA *SCHEMA::UnionSchema(SCHEMA *other) {
  ...
  for (i = 0; i < this->TableNum; i++)
    Schema->TableId[i] = this->TableId[i];
  for (j = 0; j < other->TableNum; j++)
    Schema->TableId[i + j] = other->TableId[j];
  ...
  if (FKeyList.GetSize() > 0) {
    ...
    for (i = 0; i < FKeyList.GetSize(); i++) {
      if (i < FKeyList.GetSize() - 1)
        ...
        temp.Format("%s%s", (*FKeyList[i]).Dump().c_str(), "\r\n");
      os += temp;
    }
  }
  return os;
...
DOM_TYPE atoDomain(char *p) {
  if (strcmp(p, "string_t") == 0)
    return string_t;
  if (strcmp(p, "int_t") == 0)
    return int_t;
  if (strcmp(p, "real_t") == 0)
    return real_t;
...
ORDER_AD atoKeyOrder(char *p) {
  if (strcmp(p, "ascending") == 0)
    return ascending;
  if (strcmp(p, "descending") == 0)
    return descending;
  ...
  if (other.Order == Order && *(other.Keys) == *Keys) {
    if (Order == sorted) {
      ...
      for (int i = 0; i < KeyOrder.GetSize(); i++)
        if (KeyOrder[i] != other.KeyOrder[i])
          return false;
    }
    return true;
  }
...
CString PHYS_PROP::Dump() {
  ...
  if (Order == sorted) {
    ...
    for (i = 0; i < KeyOrder.GetSize() - 1; i++) {
      temp.Format("%s, ",
                  KeyOrder[i] == ascending ? "ascending" : "descending");
      os += temp;
    }
...
CONT::CONT(PHYS_PROP *RP, COST *U, bool D)
  ...
  // If the Physical Property has >1 attribute, use only the most selective
  // attribute
  if (RP && (RP->GetKeysSet()) && (RP->GetKeysSet()->GetSize() > 1))
    // RP-> SetKeysSet( RP->GetKeysSet() -> best());
    RP->bestKey();
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_CONT].New();
};
