...
BOOL CWcolApp::InitInstance() {
  ...
  pDocTemplate =
      new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CWcolDoc),
...
void CWcolDoc::AssertValid() const { CDocument::AssertValid(); }

// ##ModelId=3B0C085B0322
void CWcolDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
void CWcolView::AssertValid() const { CEditView::AssertValid(); }

// ##ModelId=3B0C085C0002
void CWcolView::Dump(CDumpContext &dc) const { CEditView::Dump(dc); }
...
#define MAX_MEM 0xC0000000
  ...
  while ((LONG)lpScanAt < MAX_MEM) {
    ...
    if (mbi.AllocationBase != lpRegionBase) {
      ...
      if (GetModuleFileName((HINSTANCE)lpRegionBase, szModuleName, MAX_CHARS)) {
        out_string.Format(
...
BOOL COptionDialog::OnInitDialog() {
  ...
  if (!m_pToolTip) {
    ...
    MapDialogRect(rc);
    rt = m_pToolTip->AddTool(
        GetDlgItem(IDC_FILE),
        "Write all trace information to the file colout.txt");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(GetDlgItem(IDC_WINDOW),
                             "Write all trace information to the window");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(GetDlgItem(IDC_COVE),
                             "Write input for COVE (Columbia Optimizer "
                             "Visualizer Environment) to cove.txt");
    ASSERT(rc != 0);

    // rt = m_pToolTip->AddTool(this, "Option setting");
    // ASSERT( rc!=0 );

    rt = m_pToolTip->AddTool(GetDlgItem(IDC_FINALSSP),
                             "Print the final search space in colout.txt");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(GetDlgItem(IDC_OPEN),
                             "Trace all OPEN task lists to colout.txt");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(
        GetDlgItem(IDC_SSP),
        "Trace search space after every task, to colout.txt");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(GetDlgItem(IDC_PRUNING),
                             "Aggressively check limits at all times");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(
        GetDlgItem(IDC_CUCARD),
        "In addition to group pruning, use lower bound pruning");
    ASSERT(rc != 0);

    rt = m_pToolTip->AddTool(
        GetDlgItem(IDC_GBLEPS),
        "If a plan costs <= GLOBAL_EPS, it is a winner for G");
    ASSERT(rc != 0);
...
#pragma warning(                                                               \
    disable : 4800) // use of bool in wcol.cpp, casting int to bool takes time
...
void COptionDialog::OnOK() {
  ...
  if (CuCardPruning && !Pruning)
    AfxMessageBox(
        "CuCard pruning must come with group pruning option checked!");
  else
    CDialog::OnOK();
}

// ##ModelId=3B0C085C037E
void COptionDialog::OnCancel() { CDialog::OnCancel(); }
