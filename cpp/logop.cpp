...
GET::GET(CString collection, CString rangeVar) {
  ...
  if (collection == rangeVar)
    ...
    for (i = 0; i < Size; i++) // For each attribute
    {
      ...
      Cat->AddAttr(RangeVar, TruncName(GetAttName(AttArray->GetAt(i))), attr,
                   domain);
    }

    PTRACE2(
        "Catalog content after fixing AttId-based tables for range %s:\r\n%s",
    ...
    if (IndArray) {
      ...
      for (i = 0; i < Size; i++) // For each index
      {
        ...
        *indprop = *ip;
        // Alter keys in the property object so they will refer to new range
        // variable attributes
        indprop->update(RangeVar);
        Cat->AddIndex(RangeVar, TruncName(GetIndName(IndArray->GetAt(i))),
                      indprop);
      }
    }
    PTRACE2(
        "Catalog content after fixing IndId-based tables for range %s:\r\n%s",
        RangeVar.c_str(), Cat->Dump().c_str());
      ...
      for (i = 0; i < Size; i++) // For each index
      {
        ...
        *bitindprop = *ip;
        ...
        Cat->AddBitIndex(RangeVar,
                         TruncName(GetBitIndName(BitIndArray->GetAt(i))),
                         bitindprop);
      }
    }
    PTRACE2("Catalog content after fixing BitIndId-based tables for range "
            "%s:\r\n%s",
            RangeVar.c_str(), Cat->Dump().c_str());
...
LOG_PROP *GET::FindLogProp(LOG_PROP **input) {
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP(CollProp->Card, CollProp->UCard, Schema, cand_key);
...
LOG_PROP *EQJOIN::FindLogProp(LOG_PROP **input) {
  ...
  bool IsFKSchema = false; // Is it like an FK join except refkey is not a cand
                           // key of entire join
  float RefUcard = 1; // Unique card of refkey, needed to compute eqjoin card

  // Eqjoin card is card of ref input (as opposed to FK input) divided by
  // RefUcard
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP((float)Card, (float)UCard, Schema, cand_key);
...
LOG_PROP *PROJECT::FindLogProp(LOG_PROP **input) {
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP(rel_input->Card, new_ucard, schema, cand_key);
...
LOG_PROP *SELECT::FindLogProp(LOG_PROP **input) {
  ...
  for (int i = 0; i < new_schema->GetSize(); i++) {
    ...
    if (old_cucard != -1) {
      new_cucard =
          ceil(1 / (1 / old_cucard - 1 / (rel_input->Card) + 1 / new_card));
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP((float)new_card, (float)ceil(rel_input->UCard * sel),
                        new_schema, cand_key);
...
CString SELECT::Dump() { return GetName(); }

// ##ModelId=3B0C08740300
RM_DUPLICATES::RM_DUPLICATES() {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_RM_DUPLICATES].New();
#ifdef _DEBUG
  name = GetName(); // for debug
#endif
};
...
LOG_PROP *RM_DUPLICATES::FindLogProp(LOG_PROP **input) {
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP(new_card, new_ucard, new_schema, cand_key);
...
CString RM_DUPLICATES::Dump() { return GetName(); }
...
AGG_LIST::AGG_LIST(int *gby_atts, int gby_size, AGG_OP_ARRAY *agg_ops)
  ...
  if (AggOps) {
    ...
    int *TempAtts;
    for (i = 0; i < NumOps; i++)
      FAttsSize += (*AggOps)[i]->GetAttsSize();
    FlattenedAtts = new int[FAttsSize];
    index = 0;
    for (i = 0; i < NumOps; i++) {
      TempAtts =
          CopyArray((*AggOps)[i]->GetAtts(), (*AggOps)[i]->GetAttsSize());
...
LOG_PROP *AGG_LIST::FindLogProp(LOG_PROP **input) {
  ...
  for (i = 0; i < NumOps; i++) {
    ...
    ATTR *new_attr =
        new ATTR(aggop->GetRangeVar(), aggop->GetAtts(), aggop->GetAttsSize());
  ...
  LOG_COLL_PROP *result =
      new LOG_COLL_PROP(new_card, new_cucard, result_schema, cand_key);
...
LOG_PROP *FUNC_OP::FindLogProp(LOG_PROP **input) {
  ...
  LOG_COLL_PROP *result = new LOG_COLL_PROP(rel_input->Card, rel_input->UCard,
                                            result_schema, cand_key);
