...
public:
  ...
  CString Dump() {
    CString os;
    os.Format("MemUse = %d, %-16s --- Size = %d, Total = %d , Max = %d , Count "
              "= %d\r\n",
...
public:
  ...
  OPT_STAT()
      : TotalMExpr(0), DupMExpr(0), FiredRule(0), HashedMExpr(0),
        MaxBucket(0){};
...
public:
  ...
  FOREIGN_KEY(KEYS_SET *FKeys, KEYS_SET *RKeys)
      : ForeignKey(FKeys), RefKey(RKeys){};
...
public:
  ...
  ATTR(ATTR &other)
      : AttId(other.AttId), CuCard(other.CuCard), Min(other.Min),
        Max(other.Max) {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_ATTR].New();
  };
...
// is a set of attributes, plus information about which attributes are
// equivalent (attributes are made equivalent by joins on them).
...
public:
  ...
  int *TableId; // Base tables appearing in this schema - used for calculating
                // Max cucards
  //  for each base table in a schema, and for calculating log prop of
  //  aggregate.
...
public:
  ...
  SCHEMA(SCHEMA &other) : Size(other.Size), TableNum(other.TableNum) {
    ...
    Attrs = new ATTR *[Size];
    for (i = 0; i < Size; i++)
      Attrs[i] = new ATTR(*(other[i]));
    TableId = new int[TableNum];
    for (i = 0; i < TableNum; i++)
      TableId[i] = other.GetTableId(i);
  }
...
public:
  ...
  LOG_COLL_PROP(float card, float ucard, SCHEMA *schema,
                KEYS_SET *cand_keys = NULL)
      : Card(card), UCard(ucard), Schema(schema), CandidateKey(cand_keys) {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_LOG_COLL_PROP].New();
  };

  /*	LOG_COLL_PROP(LOG_COLL_PROP & other) : Card(other.Card),
     UCard(other.UCard), Schema(other.Schema)
          {
          CandidateKey = NULL;
          if (other.CandidateKey)CandidateKey = new
     KEYS_SET(*other.CandidateKey);
          };
  */
  ...
  ~LOG_COLL_PROP() {
    ...
    for (int i = 0; i < FKeyList.GetSize(); i++)
      delete FKeyList[i];
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_LOG_COLL_PROP].Delete();
  };
...
public:
  ...
  LOG_ITEM_PROP(float max, float min, float CuCard, float selectivity,
                KEYS_SET &freevars)
      : Max(max), Min(min), CuCard(CuCard), Selectivity(selectivity),
        FreeVars(freevars) {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_LOG_ITEM_PROP].New();
  };

  // ##ModelId=3B0C08630065
  ~LOG_ITEM_PROP() {
    if (TraceOn && !ForGlobalEpsPruning)
      ClassStat[C_LOG_ITEM_PROP].Delete();
  };
  ...
  CString Dump() {
    ...
    os.Format("Max : %.0f, Min : %.0f, CuCard : %.0f, Selectivity : %.3f, "
              "FreeVars : %s\r\n",
...
// Each search spawns multiple subtasks, e.g. one task to fire each rule for the
// search. These subtasks all share the search's CONT.  Sharing is done not only
// to save space, but to share information about when the search is done, what
// is the current upper bound, etc.
...
public:
  ...
  CString Dump() {
    ...
    //		os.Format("Prop: %s, UB: %s", ReqdPhys==NULL ? "ANY" :
    //ReqdPhys->Dump(), UpperBd->Dump() );
...
// for cucard pruning, calculate the minimon cost of fetching cucard tuples from
// disc
