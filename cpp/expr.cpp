...
EXPR::EXPR(OP *LogOp, EXPR *First, EXPR *Second, EXPR *Third, EXPR *Fourth)
    : Op(LogOp), arity(0) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_EXPR].New();
...
EXPR::EXPR(EXPR &Expr) : Op(Expr.GetOp()->Clone()), arity(Expr.GetArity()) {
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_EXPR].New();
  if (arity) {
    Inputs = new EXPR *[arity];
    for (int i = 0; i < arity; i++)
      Inputs[i] = new EXPR(*(Expr.GetInput(i)));
  }
}
