...
//==========
/*
This enum list is used for the rule bit vector.  It must be consistent with
NUMOFRULES and rule_set and with the rule set file read from disk. If there are
more than 32 rules, put only logical to logical transformations in the rule
vector.
*/
...
/*
   ...
   patterns.  For example, the Left to Right (LTOR) Associative Rule includes
   the following patterns, where L(i) denotes the LEAF_OP with index i (L(i) is
   essentially a multiexpression indexed by i):
                Original: (L(1) join L(2)) join L(3)
                Substitute: L(1) join (L(2) join L(3))

   One of the first steps in applying this rule is to find a multiexpression in
   the search space which binds to the Original pattern.  This is the task of
   objects in the BINDERY class.

   Suppose the multiexpression
                [ G7 join G4 ] join G10,
   where Gi is the group with GRP_ID i and [ X ] denotes the group containing X,
   is in the optimizer's search space.  Then there is a binding of the Original
   pattern to this multiexpression, given by binding L(1) to G7, L(2) to G4 and
   L(3) to G10. An object in the BINDERY class, called a bindery, will, over its
   lifetime, produce all such bindings.

   In order to produce a binding, a bindery must spawn one bindery for each
   input subgroup. For example, consider a bindery for the LTOR associativity
   rule.  It will spawn a bindery for the left input, which will seek all
   bindings to the pattern L(1) join L(2) and a bindery for the right input,
   which will seek all bindings to the pattern  L(3). In our example the right
   bindery will find only one binding, to the right input group G10.  The left
   bindery will typically find many bindings, However, the left bindery will
   find one binding for each join in the left input group, for example one
   binding for G7 join G4 and one for G4 join G7.

   Is a binding made to a multiexpression or to an expression?  In our example,
   a binding is sought for to the multiexpression [G7 join G4] join G10 .
   However, the bindings which are produced are to expressions, such as (G7 join
   G4) join G10 and (G4 join G7) join G10.  Those two are distinct as
   expressions but would be identical as multiexpressions.

   BINDERY objects (binderys) are of two types.  Expression binderys bind the
   Original pattern to only one multi-expression in a group.  A rule first
   creates an expression bindery.  This expression bindery then spawns one group
   bindery for each input group, as explained above. Group binderys bind to all
   expressions in a group.  See APPLY_RULE::perform  for details .

   Because Columbia and its predecessors apply rules only to logical
   expressions, binderys bind logical operators only.

   A bindery may produce several bindings, as mentioned above.  Thus a bindery
   may go through several stages: start, then loop over several valid bindings,
   then finish.
...
/*
  ...
  Columbia and its ancestors use only rules for which all operators in the
      original pattern are logical, and for which all operators in the
  substitute pattern are logical except perhaps the root operator.
...
private:
  ...
  EXPR *substitute; // replacement for original pattern
                    //"substitute" is used ONLY to tell if the rule is logical
                    //or physical,
                    // and, by check(), for consistency checks.  Its pattern is
                    // represented in the method next_substitute()
...
public:
  ...
  RULE(CString name, int arity, EXPR *original, EXPR *substitute)
      : name(name), arity(arity), mask(0), original(original),
