...
M_EXPR::M_EXPR(EXPR *Expr, GRP_ID grpid)
    : Op(Expr->GetOp()->Clone()), NextMExpr(NULL),
      GrpID((grpid == NEW_GRPID) ? Ssp->GetNewGrpID() : grpid), HashPtr(NULL),
      RuleMask(0) {
  GRP_ID GID;
  EXPR *input;
  counter = 0;
  ...
  if (arity) {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (input->GetOp()
              ->is_leaf()) // deal with LEAF_OP, sharing the existing group
...
M_EXPR::M_EXPR(M_EXPR &other)
    : GrpID(other.GrpID), HashPtr(other.HashPtr), NextMExpr(other.NextMExpr),
