...
MEMORY_MANAGER::MEMORY_MANAGER(int block_anchor_count, int realloc_factor) {
  ...
  _block_anchors =
      (BLOCK_ANCHOR *)malloc(block_anchor_count * sizeof(BLOCK_ANCHOR));
...
void MEMORY_MANAGER::allocate_block(BLOCK_ANCHOR &anchor) {
  ...
  *(void **)(((unsigned long)tmp) +
             (unsigned long)(anchor._next_block_offset)) = anchor._first_block;
  anchor._first_block = tmp;

  anchor._last_slot_index = 0;
  anchor._last_slot =
      (void *)((unsigned long)tmp + (unsigned long)anchor._first_slot_offset);
}
...
void *MEMORY_MANAGER::allocate(BLOCK_ANCHOR **anchor_v, int elem_size,
  ...
  if (anchor_p->_free_slot != NULL) {
    ...
    (anchor_p->_last_slot) = (void *)((unsigned long)(anchor_p->_last_slot) +
...

{
  if (_block_anchor_count == _allocated_block_anchor_count) {
    ...
    void *temp_ptr = (void *)realloc(
        (char *)_block_anchors,
        (unsigned int)(_allocated_block_anchor_count * (sizeof(BLOCK_ANCHOR))));
...
CString MEMORY_MANAGER::Dump() {
  ...
  for (int i = 0; i < _block_anchor_count; i++) {
    ...
    temp.Format("Anchor %d --- slot_size: %d, block_size: %d, NewCalls: %d, "
                "DeleteCalls: %d, ActualAlloc: %d\r\n",
                i, anchor_p->_slot_size, anchor_p->_block_size,
                anchor_p->NewCalls, anchor_p->DeleteCalls,
                anchor_p->ActualAlloc);
    os += temp;
  }
...
BLOCK_ANCHOR::~BLOCK_ANCHOR() {
  ...
  while (block_p) {
    void *temp_block_p;
    temp_block_p = *(void **)(((unsigned long)block_p) +
